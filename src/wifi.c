...
void wifi_connect(void)
{
	...
	static struct wifi_connect_req_params cnx_params = {
		.ssid = CONFIG_WIFI_SAMPLE_SSID,
		.ssid_length = 0,
		.psk = CONFIG_WIFI_SAMPLE_PSK,
		.psk_length = 0,
		.channel = 0,
		.security = WIFI_SECURITY_TYPE_PSK,
	};

	cnx_params.ssid_length = strlen(CONFIG_WIFI_SAMPLE_SSID);
	cnx_params.psk_length = strlen(CONFIG_WIFI_SAMPLE_PSK);

	connected = 0;

	LOG_INF("WIFI try connecting to %s...", CONFIG_WIFI_SAMPLE_SSID);
	...
	LOG_INF("WIFI connected %s...", CONFIG_WIFI_SAMPLE_SSID);
}
